...
export class SessionManager {
    ...
    private readonly SESSIONS_KEY = 'mindcortex.sessions';
    private readonly CURRENT_SESSION_KEY = 'mindcortex.currentSession';
    ...
    public async loadSessions(): Promise<void> {
        try {
            const sessionsData = this.context.globalState.get<Session[]>(
                this.SESSIONS_KEY,
                []
            );
            ...
            // 加载当前会话
            this.currentSessionId = this.context.globalState.get<string>(
     ...
     * 创建默认会话
     */
    private async createDefaultSession(): Promise<Session> {
    ...
    public async createSession(options: Partial<Session>): Promise<Session> {
        ...
        Logger.info(`已创建会话: ${session.name} (${session.id})`);
     ...
     * 切换会话
     */
    public async switchSession(sessionId: string): Promise<Session | undefined> {
        ...
        Logger.info(`已切换到会话: ${session.name}`);
    ...
    public async updateSession(
        ...
        Logger.info(`已更新会话: ${session.name}`);
        return session;
    }
    ...
    public async deleteSession(sessionId: string): Promise<boolean> {
        ...
        Logger.info(`已删除会话: ${session.name}`);
     ...
     * 添加标签到会话
     */
    public async addTag(sessionId: string, tag: string): Promise<void> {
        const session = this.sessions.get(sessionId);
        if (!session) {
            return;
        }
     ...
     * 从会话中移除标签
     */
    public async removeTag(sessionId: string, tag: {includes: (arg0: string) => any;}): Promise<void> {
        const session = this.sessions.get(sessionId);
        if (!session) {
            return;
        }
     ...
     * 按标签筛选会话
     */
    public filterSessionsByTag(tag: string): Session[] {
     ...
     * 管理会话（显示快速选择面板）
     */
    public async manageSessions(): Promise<void> {
     ...
     * 提示创建新会话
     */
    private async promptCreateSession(): Promise<void> {
...
: description
     ...
     * 提示切换会话
     */
    private async promptSwitchSession(): Promise<void> {
     ...
     * 提示删除会话
     */
    private async promptDeleteSession(): Promise<void> {
        const sessions = this.getAllSessions();
        if (sessions.length === 0) {
            vscode.window.showInformationMessage('没有可用的会话');
            return;
     ...
     * 导入会话
     */
    public async importSession(json: string): Promise<Session> {
        try {
            ...
            Logger.info(`已导入会话: ${session.name}`);
            return session;
        } catch (error) {
            Logger.error(`导入会话失败: ${error}`);

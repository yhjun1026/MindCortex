...
export class WebviewManager {
     ...
     * 打开主面板
     */
    public openMainPanel(): void {
        this.openWebviewPanel(
            'mindcortex.main',
            'MindCortex',
            this.getMainWebviewContent()
        );
    }
     ...
     * 打开搜索面板
     */
    public openSearchPanel(): vscode.WebviewPanel | undefined {
        return this.openWebviewPanel(
            'mindcortex.search',
            'MindCortex 搜索',
            this.getSearchWebviewContent()
        );
    }
     ...
     * 打开知识图谱
     */
    public openKnowledgeGraph(): void {
        this.openWebviewPanel(
            'mindcortex.graph',
            'MindCortex 知识图谱',
            this.getGraphWebviewContent()
        );
    }
     ...
     * 打开代码助手
     */
    public openCodeAssistant(): vscode.WebviewPanel | undefined {
        return this.openWebviewPanel(
            'mindcortex.assistant',
            'MindCortex 代码助手',
            this.getAssistantWebviewContent()
        );
    }
    ...
    private openWebviewPanel(
        ...
        panel.onDidDispose(() => {
            ...
            Logger.info(`Webview 面板已关闭: ${viewType}`);
        }, this.context.subscriptions);

        this.panels.set(viewType, panel);
        Logger.info(`Webview 面板已创建: ${viewType}`);
    ...
    private async handleWebviewMessage(
        ...
        Logger.info(`收到 Webview 消息 [${viewType}]: ${message.type}`);
    ...
    private async handleAssistantMessage(
        ...
        switch (action) {
            case 'insertCode':
                ...
                const code = (message as any).code;
                if (editor && code) {
    ...
    private async handleGraphMessage(
        ...
        // TODO: 实现图谱消息处理
        Logger.info(`图谱消息: ${JSON.stringify(message)}`);
    }
     ...
     * 获取搜索面板 HTML 内容
     */
    private getSearchWebviewContent(): string {
        return this.getCommonHtml(`
     ...
     * 获取代码助手面板 HTML 内容
     */
    private getAssistantWebviewContent(): string {
        return this.getCommonHtml(`
     ...
     * 获取知识图谱 HTML 内容
     */
    private getGraphWebviewContent(): string {
        return this.getCommonHtml(`
     ...
     * 获取通用 HTML 模板
     */
    private getCommonHtml(bodyContent: string): string {
        return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
...
<body>
     ...
     * 检查面板是否存在
     */
    public hasPanel(viewType: string): boolean {
